...
writeBedFile <- function(fileName, dataDirectory, fragments=NULL, deltaWs=NULL, breakTrack=NULL, bin=NA, summary=F) {
	
  fileDestination <- file.path(dataDirectory, "")
	
  index <- strsplit(fileName, '\\.')[[1]][1] # pulls out first element in fileName
  
  # write bedfile of reads that in fragments GRanges object 
  chr <- fragments$chromosome[1]	
